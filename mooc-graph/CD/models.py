...
class our_adaptive(nn.Module):
    ...
    def __init__(self, args, exer_n, student_n, knowledge_n, mf_type, dim, num_layer, epsilon):
        self.epsilon = epsilon
        self.layer_depth = num_layer
        self.knowledge_n = knowledge_n
        self.exer_n = exer_n
    ...
    def graph_update(self, args, predicted_results):
        ...
        :return:
        '''
        train_data_user_score_postrue, train_data_user_score_posfalse = defaultdict(set), defaultdict(set)
        train_data_user_score_negtrue, train_data_user_score_negfalse = defaultdict(set), defaultdict(set)
        train_data_item_score_postrue, train_data_item_score_posfalse = defaultdict(set), defaultdict(set)
        train_data_item_score_negtrue, train_data_item_score_negfalse = defaultdict(set), defaultdict(set)
        ...
        # I only drop a little part of un-trusted interactions!
        neg_interactions_true = neg_interactions[np.where(neg_interactions[:, 3] <= 1-self.epsilon)]
        neg_interactions_false = neg_interactions[np.where(neg_interactions[:, 3] > self.epsilon)]

        print(neg_interactions_true.shape)
        print(neg_interactions_false.shape)

        # I only drop a little part of un-trusted interactions!
        pos_interactions_true = pos_interactions[np.where(pos_interactions[:, 3] >= 0.1)]
        pos_interactions_false = pos_interactions[np.where(pos_interactions[:, 3] < 0)]


        for value in pos_interactions_false:
            user = int(value[0])
            item = int(value[1])
            train_data_user_score_posfalse[user].add(int(item))
            train_data_item_score_posfalse[int(item)].add(user)
        for value in pos_interactions_true:
            user = int(value[0])
            item = int(value[1])
            train_data_user_score_postrue[user].add(int(item))
            train_data_item_score_postrue[int(item)].add(user)
        for value in neg_interactions_false:
            user = int(value[0])
            item = int(value[1])
            train_data_user_score_negfalse[user].add(int(item))
            train_data_item_score_negfalse[int(item)].add(user)
        for value in neg_interactions_true:
            user = int(value[0])
            item = int(value[1])
            train_data_user_score_negtrue[user].add(int(item))
        ...
        # u_d_posfalse = readD(args, train_data_user_score_posfalse, args.student_n)
        # i_d_posfalse = readD(args, train_data_item_score_posfalse, args.exer_n)
        # u_d_negfalse = readD(args, train_data_user_score_negfalse, args.student_n)
        # i_d_negfalse = readD(args, train_data_item_score_negfalse, args.exer_n)
        ...
        # sparse_ui_posfalse = readTrainSparseMatrix(args, train_data_user_score_posfalse, u_d_posfalse, i_d_posfalse, True)
        # sparse_iu_posfalse = readTrainSparseMatrix(args, train_data_item_score_posfalse, u_d_posfalse, i_d_posfalse, False)
        # sparse_ui_negfalse = readTrainSparseMatrix(args, train_data_user_score_negfalse, u_d_negfalse, i_d_negfalse, True)
        # sparse_iu_negfalse = readTrainSparseMatrix(args, train_data_item_score_negfalse, u_d_negfalse, i_d_negfalse, False)
        ...
        # for i in range(len(u_d_negfalse)):
        #     u_d_negfalse[i] = [u_d_negfalse[i]]
        # for i in range(len(i_d_negfalse)):
        #     i_d_negfalse[i] = [i_d_negfalse[i]]
        #
        # for i in range(len(u_d_posfalse)):
        #     u_d_posfalse[i] = [u_d_posfalse[i]]
        # for i in range(len(i_d_posfalse)):
        #     i_d_posfalse[i] = [i_d_posfalse[i]]
        ...
        # self.d_i_train_00 = torch.cuda.FloatTensor(u_d_negfalse)
        # self.d_j_train_00 = torch.cuda.FloatTensor(i_d_negfalse)
        # self.d_i_train_10 = torch.cuda.FloatTensor(u_d_posfalse)
        # self.d_j_train_10 = torch.cuda.FloatTensor(i_d_posfalse)
        self.update = True
    ...
    def graph_representations(self):
        ...
        stu_emb_bias = self.student_emb_bias.weight
        stu_emb_bias = stu_emb_bias.reshape(-1,1)
        knowledge_base_emb = self.knowledge_emb
        ...
        if not self.update:
            for _ in range(self.layer_depth):
                ...
                gcn1_users_embedding_0 = torch.sparse.mm(self.user_item_matrix_0, k_difficulty) + stat_emb.mul(self.d_i_train_0)
                gcn1_items_embedding_0 = torch.sparse.mm(self.item_user_matrix_0, stat_emb) + k_difficulty.mul(self.d_j_train_0)
                stat_emb = stat_emb + gcn1_users_embedding_1 + gcn1_users_embedding_0
                k_difficulty = k_difficulty + gcn1_items_embedding_1 + gcn1_items_embedding_0

        else:
            for _ in range(self.layer_depth):
                gcn1_users_embedding_postrue = torch.sparse.mm(self.user_item_matrix_postrue, k_difficulty) + stat_emb.mul(self.d_i_train_11)
                gcn1_items_embedding_postrue = torch.sparse.mm(self.item_user_matrix_postrue, stat_emb) + k_difficulty.mul(self.d_j_train_11)
                # gcn1_users_embedding_posfalse = torch.sparse.mm(self.user_item_matrix_posfalse, k_difficulty) + stat_emb.mul(self.d_i_train_10)
                # gcn1_items_embedding_posfalse = torch.sparse.mm(self.item_user_matrix_posfalse, stat_emb) + k_difficulty.mul(self.d_j_train_10)
                gcn1_users_embedding_negtrue = torch.sparse.mm(self.user_item_matrix_negtrue, k_difficulty) + stat_emb.mul(self.d_i_train_01)
                gcn1_items_embedding_negtrue = torch.sparse.mm(self.item_user_matrix_negtrue, stat_emb) + k_difficulty.mul(self.d_j_train_01)
                # gcn1_users_embedding_negfalse = torch.sparse.mm(self.user_item_matrix_negfalse, k_difficulty) + stat_emb.mul(self.d_i_train_00)
                # gcn1_items_embedding_negfalse = torch.sparse.mm(self.item_user_matrix_negfalse, stat_emb) + k_difficulty.mul(self.d_j_train_00)
                stat_emb = stat_emb + gcn1_users_embedding_postrue + gcn1_users_embedding_negtrue
                k_difficulty = k_difficulty + gcn1_items_embedding_postrue  + gcn1_items_embedding_negtrue
