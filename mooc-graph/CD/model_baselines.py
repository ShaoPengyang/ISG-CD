...
class KSCD(nn.Module):
    def __init__(self, args, exer_n, student_n, knowledge_n, dim):
        ...
        self.prednet_full1 = PosLinear(self.emb_dim, int(self.emb_dim/2), bias=False)
        self.drop_1 = nn.Dropout(p=0.5)
        self.prednet_full2 = PosLinear(int(self.emb_dim/2), int(self.emb_dim/4), bias=False)
        self.drop_2 = nn.Dropout(p=0.5)
        self.prednet_full3 = PosLinear(int(self.emb_dim/4), 1)
    ...
    def forward(self, stu_id, input_exercise, input_knowledge_point):
        ...
        pdb.set_trace()
        sum_out = torch.sum(o * input_knowledge_point.unsqueeze(2), dim=1)
        count_of_concept = torch.sum(input_knowledge_point, dim=1).unsqueeze(1)
